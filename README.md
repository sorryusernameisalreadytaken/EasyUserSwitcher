...
👥 **Easy User Switcher**
> Wirelessly pair and connect to your own device’s ADB daemon, list all secondary users on the device, and switch into a selected user profile. Once the device becomes idle (screen off), aShellYou automatically switches back to the main (owner) profile. This is perfect for devices shared with family members or for testing multiple Android environments.
</br>
...
The **Easy User Switcher** feature lets you seamlessly switch between multiple user profiles on your Android device over Wireless debugging. To use it:

1. **Enable wireless debugging** on your phone: open *Settings → Developer options → Wireless debugging*.
2. On the *Wireless debugging* page, choose *Pair device with pairing code* and note the **host**, **pairing port** and **pairing code** shown on your phone.
3. Open **aShellYou** and tap the **User Switcher** card on the home screen.
4. In the **User Switcher** screen, enter the *host*, *pairing port* and *pairing code* values and tap **Pair**. Once pairing succeeds, tap **Connect** (port 5555 is used by default on most devices).
5. Tap **Load Users** to retrieve the list of available user profiles from your device.
6. Tap on a user entry to temporarily switch into that profile. aShellYou will monitor the device’s idle state and automatically return to the owner (user 0) when the screen turns off. You can manually switch back at any time.
...
<p align="start">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="docs/translations-dark.svg?ts=1768458616">
    <source media="(prefers-color-scheme: light)" srcset="docs/translations-light.svg?ts=1768458616">
